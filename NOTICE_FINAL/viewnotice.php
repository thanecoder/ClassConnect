...
if($id!='')
{
	...
	$query="SELECT * from notice WHERE id=$id ";//selecting names of cities
...
$(document).ready(function(){
	...
	//onKeyUp function gets called for every keyup event on input having id as 'keyword'
		//ajax starts
		$.ajax({
		  type: 'POST',  //method
		  url: 'autocompletetrial1.php', //target file
		  data: { q: $('#keyword').val()} //data need to be send 
		}).done(function( msg ){ 
					//done methods gets called when ajax response is sent back by jQuery ajax.
					// response is retrieved in variable 'msg'
		  $('#tp1').html(msg);  // Write the ajax response to span having id 'span' 
		});	
	});
});
</script>
	...
	<nav class='navbar  navbar-fixed-top' id='my-navbar'>
		<div class='container'>
			...
			<div  style='background-color:#CFD8DC' id='colnav'>
		...
		<!--left side wala div-->
		<div class='container-fluid' style='float:left; height:540px; 
		width:850px;  margin-top:75px;'>
			
 					<div class='modal-content' style='height:500px; width:100%; position:relative;' 
					id='dynamicdiv'>
						<div class='modal-header' id='header' style='background-color: #CFD8DC; 
							 border:1px solid #1A237E;    border-radius:5px; 
							 color:#212121; font-family:Verdana;'>$topic<!--Modal Header-->
 						</div>	
							<div id='wrapper' style='width:100%; height:240px;'> 
								<div id='content' > 
	<p style='font-family:Cambria Math; font-size:20px;'>$desc</p>
								</div>
							</div>
							<br>
							<br>
							<p style='font-family:Cambria Math; font-size:20px;'>FILE: $file</p>
							<div style='position: absolute; bottom: 0; height:55px; width:100%;'>
								...
								<form method='post' action='downloadfile.php'id='downloadform'>
								<button type ='button' class ='btn btn-info' style='margin-left:150px;
								width:100px;'>View</button>
								
								<input type='submit' value='Download' class='btn btn-primary btn-success'
								style='width:100px;'>
								<input type='hidden' name='file' id='file' value='$file'>
								</form>
								<div style='float:right; margin-top:-30px; height:60px; width:250px; 
								font-family:Cambria Math; font-size:20px; '>
		...
		<!--right side wala-->
<div style='float:left;width:480px; height:480px;  margin-top:75px; '>
